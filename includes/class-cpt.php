...
class SL_CPT {
	...
	public static function register(): void {
		register_post_type( self::POST_TYPE, [
			'labels' => [
				'name'               => __( 'Punti vendita', 'store-locator' ),
				'singular_name'      => __( 'Punto vendita', 'store-locator' ),
				'add_new'            => __( 'Aggiungi nuovo', 'store-locator' ),
				'add_new_item'       => __( 'Aggiungi punto vendita', 'store-locator' ),
				'edit_item'          => __( 'Modifica punto vendita', 'store-locator' ),
				'all_items'          => __( 'Tutti i punti vendita', 'store-locator' ),
				'menu_name'          => __( 'Store Locator', 'store-locator' ),
				'search_items'       => __( 'Cerca punti vendita', 'store-locator' ),
			],
			'public'        => false,
			'show_ui'       => true,
			'show_in_menu'  => true,
		...
		register_taxonomy( self::TAXONOMY, self::POST_TYPE, [
			'labels' => [
				'name'          => __( 'Categorie', 'store-locator' ),
				'singular_name' => __( 'Categoria', 'store-locator' ),
			],
			'public'            => false,
			'show_ui'           => true,
			'show_in_rest'      => true,
	...
	public static function add_meta_box(): void {
		add_meta_box(
			'sl_store_details',
			__( 'Dettagli punto vendita', 'store-locator' ),
	...
	public static function render_meta_box( WP_Post $post ): void {
		...
		?>
		<style>
			.sl-grid { display:grid; grid-template-columns:repeat(2,1fr); gap:12px 20px; }
			.sl-grid .sl-full { grid-column:1 / -1; }
			.sl-grid label { display:block; font-weight:600; margin-bottom:4px; }
			.sl-grid input[type=text], .sl-grid input[type=email], .sl-grid input[type=url], .sl-grid textarea {
				width:100%; box-sizing:border-box;
			}
			.sl-grid textarea { min-height:90px; }
			.sl-hint { color:#666; font-size:12px; margin-top:8px; }
		</style>
		<div class="sl-grid">
			<div class="sl-full">
				<label for="sl_address"><?php esc_html_e( 'Indirizzo', 'store-locator' ); ?></label>
				<input type="text" id="sl_address" name="_sl_address" value="<?php echo esc_attr( $get( '_sl_address' ) ); ?>">
			</div>
			<div>
				<label for="sl_cap"><?php esc_html_e( 'CAP', 'store-locator' ); ?></label>
				<input type="text" id="sl_cap" name="_sl_cap" value="<?php echo esc_attr( $get( '_sl_cap' ) ); ?>">
			</div>
			<div>
				<label for="sl_city"><?php esc_html_e( 'Città', 'store-locator' ); ?></label>
				<input type="text" id="sl_city" name="_sl_city" value="<?php echo esc_attr( $get( '_sl_city' ) ); ?>">
			</div>
			<div>
				<label for="sl_province"><?php esc_html_e( 'Provincia (sigla)', 'store-locator' ); ?></label>
				<input type="text" id="sl_province" maxlength="2" name="_sl_province" value="<?php echo esc_attr( $get( '_sl_province' ) ); ?>">
			</div>
			<div>
				<label for="sl_country"><?php esc_html_e( 'Paese (ISO 2)', 'store-locator' ); ?></label>
				<input type="text" id="sl_country" maxlength="2" name="_sl_country" value="<?php echo esc_attr( $get( '_sl_country' ) ?: 'IT' ); ?>">
			</div>
			<div>
				<label for="sl_phone"><?php esc_html_e( 'Telefono (visualizzato)', 'store-locator' ); ?></label>
				<input type="text" id="sl_phone" name="_sl_phone_raw" value="<?php echo esc_attr( $get( '_sl_phone_raw' ) ?: $get( '_sl_phone' ) ); ?>">
			</div>
			<div>
				<label for="sl_phone_tel"><?php esc_html_e( 'Telefono (formato tel:)', 'store-locator' ); ?></label>
			...
			<div>
				<label for="sl_website"><?php esc_html_e( 'Sito web', 'store-locator' ); ?></label>
				<input type="url" id="sl_website" name="_sl_website" value="<?php echo esc_attr( $get( '_sl_website' ) ); ?>">
			</div>
			<div class="sl-full">
				<label for="sl_hours"><?php esc_html_e( 'Orari', 'store-locator' ); ?></label>
				<textarea id="sl_hours" name="_sl_hours"><?php echo esc_textarea( $get( '_sl_hours' ) ); ?></textarea>
			</div>
			<div>
				<label for="sl_lat"><?php esc_html_e( 'Latitudine', 'store-locator' ); ?></label>
				<input type="text" id="sl_lat" name="_sl_lat" value="<?php echo esc_attr( $get( '_sl_lat' ) ); ?>">
			</div>
			<div>
				<label for="sl_lng"><?php esc_html_e( 'Longitudine', 'store-locator' ); ?></label>
				<input type="text" id="sl_lng" name="_sl_lng" value="<?php echo esc_attr( $get( '_sl_lng' ) ); ?>">
			</div>
			<div>
				<label for="sl_external_id"><?php esc_html_e( 'ID esterno (CodCli)', 'store-locator' ); ?></label>
				<input type="text" id="sl_external_id" name="_sl_external_id" value="<?php echo esc_attr( $get( '_sl_external_id' ) ); ?>">
			</div>
			<div>
				<label for="sl_category"><?php esc_html_e( 'Categoria', 'store-locator' ); ?></label>
				<input type="text" id="sl_category" name="_sl_category" value="<?php echo esc_attr( $get( '_sl_category' ) ); ?>">
			</div>
		</div>
		<p class="sl-hint"><?php esc_html_e( 'Se lat/lng sono vuoti al salvataggio, l\'indirizzo verrà geocodificato automaticamente tramite Google.', 'store-locator' ); ?></p>
		<?php
	}

	public static function save_meta( int $post_id, WP_Post $post ): void {
		if ( ! isset( $_POST['sl_meta_nonce'] ) ||
